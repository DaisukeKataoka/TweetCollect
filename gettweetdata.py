...
class CollectTweetDate():
    def init(fgc=500, tgc=100):
        dbname = "twitter.sqlite3"
        fname = "settings.yml"
        with open(fname) as rf:
            text = rf.read()
            Keys = yaml.load(text)
        Tweet_Get_Count = fgc #<-件のtweetを取得 Max100
        Follower_Get_Count = tgc #半端なくフォロワー多い人でも取得するが、API処理の関係で1200件まで
    ...
    def get_json_cache(self, curs, url, flag="tweet"):
    ...
    def get_dbdata(self, target_column, table_name, key, value):
    ...
    def get_dbdata2(self, target_column, table_name, key1, value1, key2, value2):
        with sqlite3.connect(dbname, isolation_level=None) as conn:
            curs = conn.cursor()
    ...
    def get_tweet(self, search_word, max_id, since_id):
    ...
    def get_profiles(self, follower_ids):
    ...
    def get_followee(self, user_id): #15 at once
    ...
    def get_follower(self, user_id): #5000 at once
    ...
    def get_user_id_from_tweet_id(self, tweet_ids):
    ...
    def get_past_tweet(self, user_id, base_tweet_id):
    ...
    def insert_query(self, q_id, query):
...
if __name__ == '__main__':
    for key, value in read_query().items():
